import { makeCreateTaskReclassificationRequestApiPath } from 'features/tasks/api/helpers'
import { CreateTaskReclassificationRequestResponse } from 'features/tasks/api/schemas'

import { HttpMethodEnum } from 'shared/constants/http'
import { IdType } from 'shared/types/common'

import { getRequestMockFn, getSuccessMockFn } from '_tests_/mocks/api/request'
import { ResponseResolverOptions } from '_tests_/mocks/api/response'

const createTaskReclassificationRequestMockFn = (id: IdType) =>
  getRequestMockFn(HttpMethodEnum.Post, makeCreateTaskReclassificationRequestApiPath(id))

export const mockCreateTaskReclassificationRequestSuccess = (
  id: IdType,
  options?: Partial<ResponseResolverOptions<CreateTaskReclassificationRequestResponse>>,
) => getSuccessMockFn(createTaskReclassificationRequestMockFn(id), options)()
