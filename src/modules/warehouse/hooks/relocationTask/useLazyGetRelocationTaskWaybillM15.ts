...
import { getRelocationTaskWaybillM15Messages } from 'modules/warehouse/constants/relocationTask'
import {
  ...
  useEffect(() => {
    if (isErrorResponse(state.error)) {
      if (isForbiddenError(state.error) && state.error.data.detail) {
        ...
        showErrorNotification(getRelocationTaskWaybillM15Messages.commonError)
      }
    }
  }, [state.error])
