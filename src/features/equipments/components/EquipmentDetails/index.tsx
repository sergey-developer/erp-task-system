...
import { getEquipmentFormInitialValues } from 'features/equipments/components/EquipmentFormModal/utils'
...
import { EquipmentCategoryDTO } from '../../api/dto'
import { CreateEquipmentTechnicalExaminationResponse } from '../../api/schemas'
...
const EquipmentRelocationHistoryModal = React.lazy(
  () => import('features/equipments/components/EquipmentRelocationHistoryModal'),
)
...
const CreateEquipmentTechnicalExaminationModal = React.lazy(
  () => import('features/equipments/components/CreateEquipmentTechnicalExaminationModal'),
)
...
const EquipmentDetails: FC<EquipmentDetailsProps> = ({ equipmentId, ...props }) => {
  ...
  const onCreateTechnicalExamination: CreateEquipmentTechnicalExaminationModalProps['onSubmit'] =
    async (values, setFields) => {
      ...
      if (createdTechnicalExamination) {
        try {
          const response = await getTechnicalExaminationPdf({
            technicalExaminationId: createdTechnicalExamination.id,
          }).unwrap()
