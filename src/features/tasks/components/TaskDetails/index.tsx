...
} from 'features/tasks/components/CreateInternalTaskModal/types'
...
} from 'features/tasks/components/RequestTaskSuspendModal/types'
...
import { UpdateTaskDeadlineModalProps } from 'features/tasks/components/UpdateTaskDeadlineModal/types'
import { UpdateTaskDescriptionModalProps } from 'features/tasks/components/UpdateTaskDescriptionModal/types'
import {
  TaskDetailsTabsEnum,
  taskImpactMap,
  taskPriorityMap,
...
const CreateRegistrationFNRequestModal = React.lazy(
  () => import('features/tasks/components/CreateRegistrationFNRequestModal'),
)

const ConfirmCancelReclassificationRequestModal = React.lazy(
  () => import('features/tasks/components/ConfirmCancelReclassificationRequestModal'),
)

const ExecuteTaskModal = React.lazy(() => import('features/tasks/components/ExecuteTaskModal'))

const ConfirmExecuteTaskReclassificationTasksModal = React.lazy(
  () => import('features/tasks/components/ConfirmExecuteTaskReclassificationTasksModal'),
)

const ConfirmExecuteTaskRegistrationFNModal = React.lazy(
  () => import('features/tasks/components/ConfirmExecuteTaskRegistrationFNModal'),
)

const TaskSuspendRequest = React.lazy(() => import('features/tasks/components/TaskSuspendRequest'))

const RequestTaskReclassificationModal = React.lazy(
  () => import('features/tasks/components/RequestTaskReclassificationModal'),
)

const TaskReclassificationRequest = React.lazy(
  () => import('features/tasks/components/TaskReclassificationRequest'),
)

const RequestTaskSuspendModal = React.lazy(
  () => import('features/tasks/components/RequestTaskSuspendModal'),
)

const UpdateTaskDescriptionModal = React.lazy(
  () => import('features/tasks/components/UpdateTaskDescriptionModal'),
)

const UpdateTaskDeadlineModal = React.lazy(
  () => import('features/tasks/components/UpdateTaskDeadlineModal'),
)

const CreateInternalTaskModal = React.lazy(
  () => import('features/tasks/components/CreateInternalTaskModal'),
)
...
const TaskDetails: FC<TaskDetailsProps> = ({
    ...
    async (values) => {
      try {
        ...
        if (isErrorResponse(error) && isNotFoundError(error)) {
          ...
          showErrorNotification(getTaskWorkPerformedActErrorMessage)
        }
      }
    },
  ...
  const onUpdateAssignee = useCallback(
    async (assignee: TaskAssigneeDTO['id']) => {
