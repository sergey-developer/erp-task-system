...
} from 'features/relocationTasks/api/endpoints/relocationTasks.endpoints'
...
import { UpdateExternalRelocationRequest } from '../../api/schemas'
import {
...
const CancelRelocationTaskModal = React.lazy(
  () => import('features/relocationTasks/components/CancelRelocationTaskModal'),
)

const ExecuteRelocationTaskModal = React.lazy(
  () => import('features/relocationTasks/components/ExecuteRelocationTaskModal'),
)

const ReturnRelocationTaskToReworkModal = React.lazy(
  () => import('features/relocationTasks/components/ReturnRelocationTaskToReworkModal'),
)

const ConfirmExecutionRelocationTaskModal = React.lazy(
  () => import('features/relocationTasks/components/ConfirmExecutionRelocationTaskModal'),
)

const ConfirmMoveRelocationTaskDraftToWorkModal = React.lazy(
  () => import('features/relocationTasks/components/ConfirmMoveRelocationTaskDraftToWorkModal'),
)
      ...
      if (isErrorResponse(error)) {
        if (error.data.detail) {
          if (isBadRequestError(error)) {
            ...
            showErrorNotification(cancelRelocationTaskErrorMessage)
          }
        }
      }
    }
      ...
      if (isErrorResponse(error)) {
        if (error.data.detail) {
          if (isBadRequestError(error)) {
            ...
            showErrorNotification(closeRelocationTaskErrorMessage)
          }
        }
      }
    }
    ...
    try {
      ...
      if (isErrorResponse(error)) {
        if (isBadRequestError(error)) {
          ...
          showErrorNotification(returnRelocationTaskToReworkErrorMessage)
        }
      }
    }
  }
    ...
    try {
      ...
      if (isErrorResponse(error)) {
        if (isBadRequestError(error)) {
          ...
          showErrorNotification(executeRelocationTaskErrorMessage)
        }
      }
    }
  }
